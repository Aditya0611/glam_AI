...
# Makeup presets — mirrors src/data/makeupPresets.js
MAKEUP_PRESETS = [
    {
        "id": "natural",
        "name": "✨ Natural Glow",
...
@router.get("/presets")
def get_presets():
    """Returns all makeup presets."""
    return JSONResponse(content=MAKEUP_PRESETS)


@router.get("/presets/{preset_id}")
def get_preset_by_id(preset_id: str):
    """Returns a single preset by ID."""
    preset = next((p for p in MAKEUP_PRESETS if p["id"] == preset_id), None)
    if not preset:
